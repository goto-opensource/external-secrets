...
const (
	...
	errInvalidSecretStoreRef  = "secretStoreRef"
)
...
func (p *Provider) ValidateStore(store esv1beta1.GenericStore) error {
	...
	if prov.Auth.SecretRef != nil {
		if !utils.ValidateSecretSelector(store, prov.Auth.SecretRef.AccessKeyID) {
			return utils.ErrInvalidSecretKeySelector
		}
		if !utils.ValidateSecretSelector(store, prov.Auth.SecretRef.SecretAccessKey) {
			return utils.ErrInvalidSecretKeySelector
		}
	}
	...
	if prov.Auth.JWTAuth != nil && prov.Auth.JWTAuth.ServiceAccountRef != nil {
		if !utils.ValidateServiceAccountSelector(store, *prov.Auth.JWTAuth.ServiceAccountRef) {
			return utils.ErrInvalidServiceAccountSelector
		}
	}
